...
import 'package:karing/features/moneyfly/moneyfly_home_header.dart';
...
class _HomeScreenState extends LasyRenderingState<HomeScreen>
  ...
  Future<void> onTapShowAddProfile() async {
    await onTapSyncManagedProfile();
    setState(() {});
  }

  Future<void> onTapSyncManagedProfile() async {
    final account = context.read<MoneyflyAccountController>();
    try {
      await account.refreshAccount(syncConfig: true);
      await checkAndReload("onTapSyncManagedProfile");
      if (!context.mounted) {
        return;
      }
      final noConfig = ServerManager.getConfig().getServersCount(false) == 0;
      if (noConfig) {
        await DialogUtils.showAlertDialog(
          context,
          '暂未获取到可用节点，请检查套餐、到期时间或设备数量。',
        );
      }
    } catch (err) {
      if (!context.mounted) {
                      ...
                      children: [
                        ...
                        Padding(
                          padding: EdgeInsets.symmetric(
                            horizontal: windowSize.width > 720 ? 28 : 16,
                          ),
                          child: const MoneyflyHomeHeader(),
                        ),
                        const SizedBox(
                          height: 14,
                        ),
  ...
  List<Widget> createToolbar() {
    ...
    headerWidgets.add(Header('套餐购买', Icons.shopping_bag_outlined, () {
      Navigator.push(
          context,
          MaterialPageRoute(
              builder: (context) => const MoneyflyPurchaseScreen()));

...
val signingProperties =
        Properties().apply {
            if (signingPropertiesFile.exists()) {
                signingPropertiesFile.inputStream().use(this::load)
            }
        }
val hasReleaseSigning =
        signingPropertiesFile.exists() &&
                signingProperties.getProperty("storeFile.release").orEmpty().isNotEmpty()
            ...
            if (hasReleaseSigning) {
                signingConfig =
                        signingConfigs.create("profile") {
                            storeFile =
                                    rootProject.file(
                                            signingProperties.getProperty("storeFile.release")
                                    )
                            storePassword = signingProperties.getProperty("storePassword.release")
                            keyAlias = signingProperties.getProperty("keyAlias.release")
                            keyPassword = signingProperties.getProperty("keyPassword.release")
                        }
            } else {
